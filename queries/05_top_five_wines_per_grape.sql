...
WITH ranked_wines AS ( ... )
This tells the database to run the massive query inside the parentheses first 
and store the results in a temporary, invisible box named ranked_wines. 
We do this so we can easily filter the results later.
...
FROM wines JOIN ...
WHERE grapes.name IN ('Cabernet Sauvignon', 'Merlot', 'Chardonnay')
The JOIN commands link the tables together like a chain to trace a specific wine to its region, 
then to its country, and finally to that country's most used grapes. 
The WHERE clause drops millions of rows instantly, only allowing our Top 3 grapes to enter the ranking engine.

4. The Final Output 
SELECT grape_name, wine_name, ratings_average FROM ranked_wines WHERE rank <= 5;
We open our temporary ranked_wines box, select the columns, and apply one final rule: 
only print the row if our mathematically generated rank is 5 or less.
